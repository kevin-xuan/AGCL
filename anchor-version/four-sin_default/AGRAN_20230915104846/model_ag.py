...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, queries, keys, time_mask, attn_mask, time_matrix_K, time_matrix_V, dis_matrix_K, dis_matrix_V, abs_pos_K, abs_pos_V, user_bias=None):
...
class AGRAN_anchor(torch.nn.Module):
    ...
    def forward(self, user_ids, log_seqs, time_matrices, dis_matrices, pos_seqs, neg_seqs, anchor_idx, anchor_idx_tem, time_adj_matrix=None, dis_adj_matrix=None, interaction_matrix=None):
        ...
        fin_logits = fin_logits + bias + user_bias
