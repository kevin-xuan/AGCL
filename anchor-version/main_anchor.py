...
if __name__ == '__main__':
    ...
    # interaction_matrix = sp.load_npz('../data/%s_interaction_kl_notest.npz' % args.dataset)  # M x N
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        time_adj_matrix_anchor = time_adj_matrix[anchor_idx,:].todense() if type(anchor_idx) is np.ndarray else time_adj_matrix[anchor_idx.numpy(), :].todense() # (r, N)
        ...
        for step, instance in tqdm(enumerate(dataloader), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            item_embs, pos_logits, neg_logits, fin_logits, support, contra_loss = model(u, seq, time_matrix, dis_matrix, pos, neg, anchor_idx, time_adj_matrix, dis_adj_matrix)
            ...
            time_regular = kl_loss(torch.log(torch.softmax(mask(support.transpose(1,0)),dim=-1)+1e-9),torch.softmax(mask(time_prior),dim=-1))
            ...
            loss += args.tra_kl_reg * tra_regular + args.dis_kl_reg * dis_regular + args.time_kl_reg * time_regular + args.contra_reg * contra_loss

            loss.backward()
            adam_optimizer.step()
        print('\nTrain epoch:%d, time: %f(s), rec_loss: %.4f, tra_kl_loss: %.4f, dis_kl_loss: %.4f, time_kl_loss: %.4f, contra_loss: %.4f'
                % (epoch, T, np.array(rec_losses).mean(), np.array(tra_kl_losses).mean(), np.array(dis_kl_losses).mean(), np.array(time_kl_losses).mean(), np.array(contra_losses).mean()))
        f.write('\nTrain epoch:%d, time: %f(s), rec_loss: %.4f, tra_kl_loss: %.4f, dis_kl_loss: %.4f, time_kl_loss: %.4f, contra_loss: %.4f'
                  % (epoch, T, np.array(rec_losses).mean(), np.array(tra_kl_losses).mean(), np.array(dis_kl_losses).mean(), np.array(time_kl_losses).mean(), np.array(contra_losses).mean()))
        ...
        if epoch % args.valid_epoch == 0:
            ...
            ### for test ###
            NDCG, HR = evaluate_test(model, dataset, args, time_adj_matrix, dis_adj_matrix)
