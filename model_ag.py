...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, queries, keys, time_mask, attn_mask, time_matrix_K, time_matrix_V, dis_matrix_K, dis_matrix_V, abs_pos_K, abs_pos_V):
        Q, K, V = self.Q_w(queries), self.K_w(keys), self.V_w(keys)  # (B, N, D)

        Q_ = torch.cat(torch.split(Q, self.head_size, dim=2), dim=0)  # [(B, N, D // h), ...] -> (B * h, N, D // h)
        K_ = torch.cat(torch.split(K, self.head_size, dim=2), dim=0)
        V_ = torch.cat(torch.split(V, self.head_size, dim=2), dim=0)

        time_matrix_K_ = torch.cat(torch.split(time_matrix_K, self.head_size, dim=3), dim=0)  # [(B, N, N, D // h), ...] -> (B * h, N, N, D // h)
        ...
        abs_pos_K_ = torch.cat(torch.split(abs_pos_K, self.head_size, dim=2), dim=0)  # (B * h, N, D // h)
        abs_pos_V_ = torch.cat(torch.split(abs_pos_V, self.head_size, dim=2), dim=0)

        attn_weights = Q_.matmul(torch.transpose(K_, 1, 2))  # equation (15-16)
        ...
        time_mask = time_mask.unsqueeze(-1).repeat(self.head_num, 1, 1)  # (B * h, N, 1)
        time_mask = time_mask.expand(-1, -1, attn_weights.shape[-1])  # (B * h, N, N)
        attn_mask = attn_mask.unsqueeze(0).expand(attn_weights.shape[0], -1, -1)  # (B * h, N, N)
        paddings = torch.ones(attn_weights.shape) * (-2**32+1)
        paddings = paddings.to(self.dev)
        attn_weights = torch.where(time_mask, paddings, attn_weights)  # sequence len padding
        attn_weights = torch.where(attn_mask, paddings, attn_weights)  # future information padding

        attn_weights = self.softmax(attn_weights)

        attn_weights = self.dropout(attn_weights)

        outputs = attn_weights.matmul(V_)  # equation (14)
        ...
        outputs = torch.cat(torch.split(outputs, Q.shape[0], dim=0), dim=2)  # (B * h, N, D // 2) -> (B, N, D)
...
class AGRAN(torch.nn.Module):
    def __init__(self, user_num, item_num, time_num, args):
        ...
        self.user_num = user_num  # 4638
        self.item_num = item_num  # 9731
        self.dev = args.device

        self.item_emb = torch.nn.Embedding(self.item_num+1, args.hidden_units, padding_idx=0)  # (N, D) D=64, loc_id starts from 1
        ...
        self.time_matrix_K_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)  # (256+1, D) why +1? start from 1
        self.time_matrix_V_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)  # (256+1, D)
        ...
        for _ in range(args.num_blocks):  # 3 transformer block
    ...
    def seq2feats(self, user_ids, log_seqs, time_matrices, dis_matrices, item_embs):
        ...
        abs_pos_K = self.abs_pos_K_emb(positions)  # (B, N, D)
        ...
        time_matrices = torch.LongTensor(time_matrices).to(self.dev)  # (B, N, N)
        ...
        dis_matrices = torch.LongTensor(dis_matrices).to(self.dev)  # (B, N, N)
        ...
        timeline_mask = torch.BoolTensor(log_seqs == 0).to(self.dev)  #* (B, N), padding sequence to maxlen
        seqs *= ~timeline_mask.unsqueeze(-1)  # (B, N, d) perserve useful locations information

        tl = seqs.shape[1]
        attention_mask = ~torch.tril(torch.ones((tl, tl), dtype=torch.bool, device=self.dev))  # upper triangular mask, i.e., mask future information

        for i in range(len(self.attention_layers)):  # 3 
            Q = self.attention_layernorms[i](seqs)  # layernorm
            mha_outputs = self.attention_layers[i](Q, seqs,  #* why key=seqs is not layer normalized?
            ...
            seqs = self.forward_layers[i](seqs)   # (B, N, D)
    ...
    def forward(self, user_ids, log_seqs, time_matrices, dis_matrices, pos_seqs, neg_seqs):
        ...
        item_embs, support = self.gcn(self.item_emb)  # updated loc embedding and unnormalized matrix used in Equation (11)

        self.item_embs = item_embs
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices, dis_matrices, item_embs)  # (B, N, D)

        pos_embs = item_embs[torch.LongTensor(pos_seqs).to(self.dev),:]  # (B, N, D)
        neg_embs = item_embs[torch.LongTensor(neg_seqs).to(self.dev),:]

        pos_logits = (log_feats * pos_embs).sum(dim=-1)  # (B, N)
        neg_logits = (log_feats * neg_embs).sum(dim=-1)

        fin_logits = log_feats.matmul(item_embs.transpose(0,1))  # (B, N, N_all)
